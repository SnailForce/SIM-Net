...
class KPDetector(nn.Module):
    ...
    def __init__(self, block_expansion, num_kp, num_channels, max_features,
        ...
        self.kp = torch.nn.Sequential(
             nn.Conv2d(in_channels=self.predictor.out_filters, out_channels=num_kp, kernel_size=(7, 7),
                            padding=pad)
        )
        ...
        if estimate_jacobian:
            ...
            self.jacobian[-1].weight.data.zero_()
            self.jacobian[-1].bias.data.copy_(torch.tensor([1, 0, 0, 1] * self.num_jacobian_maps, dtype=torch.float))
        else:
            self.jacobian = None
